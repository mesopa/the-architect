;(function (window, document) {
  ...
  jScaler.process = function (config) {
    ...
    if (imgs.length > 0) {
      ...
      imgs.forEach(function (img) {
        img.addEventListener('error', onerrorImg, false);  
    ...
    if (backgroundImgs.length > 0) {
      ...
      backgroundImgs.forEach(function (img) {
        if (this.attr(img, 'ci-img-index') >= 0) {  
  ...
  jScaler.processImage = function (img) {
    var sourceUrl    = 'ci-src',
        ...
        isResponsive = this.attr(img, 'ci-responsive') !== null, 
  ...
  jScaler.processBackgroundImage = function (elem) {
    var sourceUrl    = 'ci-img-background',
        isResponsive = this.attr(elem, 'ci-responsive') !== null, 
  ...
  jScaler.setUrlFromElemProperty = function (elem, sourceUrl) {
    elem.setAttribute(sourceUrl, window.getComputedStyle(elem).backgroundImage.slice(5, -2));   
  };
  ...
  function isImgSizeIsObject(imgSize) {
    return imgSize.length > 2 && imgSize.indexOf('{') === 0 && imgSize.indexOf('}') === imgSize.length - 1; 
    // rectify
  }
